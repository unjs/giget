...
- `options`: (object) Options are usually inferred from the input string. You can customize them.
  ...
  - `retryStatusCodes`: (array) Response statuses worth trying again. Defaults to `[408, 409, 425, 429, 500, 502, 503, 504]` — the set [ofetch](https://github.com/unjs/ofetch) retries. Pass a list to extend or narrow it; some hosts express a throttle with a status that is normally deterministic, GitLab's repository-archive limit being one, which can answer `406`.
